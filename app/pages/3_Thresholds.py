...
from core.thresholds import REGISTRY, validate
...
for key, spec in REGISTRY.items():
    ...
    if spec.widget == "slider":
        values[key] = st.slider(
            spec.label, spec.minimum, spec.maximum, current, spec.step, key=f"threshold_{key}"
        )
    else:
        values[key] = st.number_input(
            spec.label, min_value=spec.minimum, max_value=spec.maximum,
            value=current, step=spec.step, key=f"threshold_{key}",
        )
